...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", type=str, default="./data_t0/diagnostics_combo_signal")
...
def add_movement_budget(d: pd.DataFrame, p: SignalParams) -> pd.DataFrame:
    """Add a range/volatility budget, not a forecast of expected return."""
    x = d.copy()
...
def add_state_bins(signal: pd.DataFrame) -> pd.DataFrame:
    ...
    x["movement_budget_bin"] = qbin(x["movement_budget_bp"].replace([np.inf, -np.inf], np.nan), ["low", "mid", "high"])
    ...
    for i, (_, day) in enumerate(groups, 1):
        ...
        for sp in sig_pos:
            ...
            base = {
                ...
                "movement_budget_bp": float(sig["movement_budget_bp"]) if np.isfinite(sig["movement_budget_bp"]) else np.nan,
            }
...
def summarise(df: pd.DataFrame, group_cols: List[str]) -> pd.DataFrame:
    ...
    out = (
        ...
        .agg(
            ...
            avg_movement_budget_bp=("movement_budget_bp", "mean"),
...
def main() -> int:
    ...
    print("Adding movement budget (not expected return)...")
    ...
    by_state = summarise(raw, ["horizon", "daily_state", "daily_direction", "rel_open_amount_bin", "or_range_bin", "movement_budget_bin"])
