...
# -*- coding: utf-8 -*-
"""
build_t0_intraday_bar_panel_v2.py
...
运行示例：
    python ".\\build_t0_intraday_bar_panel_v2.py" `
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--universe-file", type=str, default="./data_t0/config/t0_etf_selected_30_detail_final.csv", help="ETF 池文件。必须包含 ts_code，最好包含 t0_category。")
...
def add_time_features(d: pd.DataFrame, entry_start: str, entry_end: str, force_flat: str) -> pd.DataFrame:
    ...
    # 简单计算距离收盘分钟数。先转成当天分钟数，避免日期基准差异。
    hh = x["clock_time"].str.slice(0, 2).astype(int)
    mm = x["clock_time"].str.slice(3, 5).astype(int)
    cur_min = hh * 60 + mm
    close_min = 15 * 60
    x["minutes_to_close"] = (close_min - cur_min).clip(lower=0)
...
def build_panel(raw: pd.DataFrame, universe: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    # v2：不用 groupby.apply 直接返回，避免新版 pandas 在 apply 后丢失 ts_code/trade_date。
    intraday_parts = []
    for (code, trade_date), g in d.groupby(["ts_code", "trade_date"], sort=False):
        x = add_intraday_features(g)
        if "ts_code" not in x.columns:
            x["ts_code"] = code
        if "trade_date" not in x.columns:
            x["trade_date"] = trade_date
        intraday_parts.append(x)
    d = pd.concat(intraday_parts, ignore_index=True, sort=False)
    d = d.sort_values(["ts_code", "trade_time"]).reset_index(drop=True)

    if "ts_code" not in d.columns or "trade_date" not in d.columns:
        raise RuntimeError(
            "构建日内特征后缺少 ts_code 或 trade_date。"
            "这是 pandas groupby/apply 兼容性问题，请使用 v2 脚本。"
        )
...
def main() -> int:
    ...
    # v2：强制检查 universe 与 raw 文件是否一致。
    # 如果这里不检查，旧的 universe 文件和新的下载池不一致时，会悄悄只加载一部分 ETF。
    missing_files = file_summary[file_summary["status"].astype(str).str.startswith("missing")]
    if not missing_files.empty:
        missing_path = out_dir / "t0_missing_raw_files.csv"
        missing_files.to_csv(missing_path, index=False, encoding="utf-8-sig")
        raise RuntimeError(
            f"universe 中有 {len(missing_files)} 只 ETF 没有找到对应分钟 parquet。"
            f"已输出缺失清单：{missing_path}。"
            f"这通常说明你用了旧的 t0_etf_selected_30_detail.csv，"
            f"请改用 t0_etf_selected_30_detail_final.csv。"
        )

    raw = normalize_raw(raw)
    raw_codes = set(raw["ts_code"].astype(str).unique())
    universe_codes = set(codes)
    if raw_codes != universe_codes:
        missing = sorted(universe_codes - raw_codes)
        extra = sorted(raw_codes - universe_codes)
        mismatch = pd.DataFrame({
            "type": ["missing_in_raw"] * len(missing) + ["extra_in_raw"] * len(extra),
            "ts_code": missing + extra,
        })
        mismatch_path = out_dir / "t0_universe_raw_mismatch.csv"
        mismatch.to_csv(mismatch_path, index=False, encoding="utf-8-sig")
        raise RuntimeError(
            f"universe ETF 数量与实际 raw ETF 数量不一致："
            f"universe={len(universe_codes)}, raw={len(raw_codes)}。"
            f"已输出：{mismatch_path}"
        )
