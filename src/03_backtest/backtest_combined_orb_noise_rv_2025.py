...
# -*- coding: utf-8 -*-
"""
backtest_combined_orb_noise_rv_2025.py

用途：
    用现有 2025 年 T+0 ETF 1min 面板，快速测试组合策略：
...
为什么写这个脚本：
    ...
    所以现在先用 2025 年旧面板做一个“组合策略原型测试”，看组合过滤后
    能否提高平均单笔毛收益和成本承受能力。
...
输出：
    默认写入：
        .\\data_t0\\backtest_combined_orb_noise_rv_2025\\

运行默认参数：
    python ".\\backtest_combined_orb_noise_rv_2025.py" `
      --panel-file ".\\data_t0\\processed\\t0_intraday_bar_panel.parquet" `
      --out-dir ".\\data_t0\\backtest_combined_orb_noise_rv_2025"

运行小网格：
    python ".\\backtest_combined_orb_noise_rv_2025.py" `
      --panel-file ".\\data_t0\\processed\\t0_intraday_bar_panel.parquet" `
      --out-dir ".\\data_t0\\backtest_combined_orb_noise_rv_2025_grid" `
      --run-grid
...
class ComboParams:
    ...
    # Range/volatility budget relative to cost; this is not expected return.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Backtest combined ORB + noise + relative-value filter strategy on 2025 panel.")
    p.add_argument("--panel-file", type=str, default="./data_t0/processed/t0_intraday_bar_panel.parquet")
    p.add_argument("--out-dir", type=str, default="./data_t0/backtest_combined_orb_noise_rv_2025")
    ...
    p.add_argument("--movement-cost-mult", "--expected-edge-mult", dest="movement_cost_mult",
                   type=float, default=1.50,
                   help="开盘区间/噪声运动空间相对假设成本的倍数；旧参数名仅为兼容。")
...
def make_params(args: argparse.Namespace) -> List[ComboParams]:
    if not args.run_grid:
        return [ComboParams(
            ...
            movement_cost_mult=args.movement_cost_mult,
    ...
    grid = {
        ...
        "movement_cost_mult": [1.0, 1.5, 2.0],
...
def simulate_trade(day: pd.DataFrame, signal_pos: int, p: ComboParams) -> Dict:
    ...
    result = simulate_long_exit(
        ...
        opening_range_stop_price=or_stop_price, stop_slippage_bp=p.stop_slippage_bp,
    )
...
def objective(trades: pd.DataFrame, cost_bp: float) -> float:
    """Conservative dev score: one-sided 90% lower net-return bound by day."""
    if len(trades) < 30 or trades["trade_date"].nunique() < 20:
        return -1e9
    net_bp = (trades["gross_ret"] - 2.0 * cost_bp / 10000.0) * 10000.0
    _, lower, _ = clustered_mean_lower_bound(net_bp, trades["trade_date"].astype(str))
    return float(lower) if np.isfinite(lower) else -1e9
...
def main() -> int:
    ...
    print("=" * 100)
    print("Combined ORB + Noise + Relative Value strategy backtest on 2025 panel")
    print("=" * 100)
    ...
    for idx, p in enumerate(params_list, 1):
        ...
        for scope in scopes:
            ...
            score = objective(dev_tr, args.main_cost_bp)
    ...
    deployment_eligible = bool(best_score > 0)
    trades = best["trades"]
    ...
    best_info = {
        ...
        "dev_score_definition": "one-sided 90% lower bound of net bp, clustered by trade date",
        "deployment_eligible": deployment_eligible,
        "selection_status": "passes_dev_gate" if deployment_eligible else "all_dev_candidates_failed_positive_lcb_gate",
        "dev_start": dev_start,
        "dev_end": dev_end,
    ...
    lines.append("")
    lines.append("## 3. 开发期候选与参数\n")
    lines.append(f"- 开发期下置信界最高的 scope: **{best_scope}**")
    lines.append(f"- 按交易日聚类的90%净收益下置信界: **{best_score:.2f}bp**")
    lines.append(f"- 是否通过开发期正下界门槛: **{deployment_eligible}**")
    ...
    if not etf_sum.empty:
        ...
        lines.append("")
    lines.append("## 7. 判断标准\n")
    lines.append("- 首先要求开发期按交易日聚类的90%净收益下界大于0；未通过时，测试结果只作诊断，不得解释为已选中策略。")
    lines.append("- 如果新的锁定 test 期 2bp 和 3bp 后仍为正，才说明组合过滤有继续开发价值。")
    lines.append("- 如果仍只在 0bp/1bp 有效，则说明当前 2025 池子上的 long-only 日内策略边际仍偏薄。")
    lines.append("- 如果交易数太少，即使收益好，也不能直接视为稳定策略。")
    (out_dir / "combo_report.md").write_text("\n".join(lines), encoding="utf-8")

    print("\n" + "=" * 100)
    print("Finished")
    print(f"diagnostic scope : {best_scope}; deployment_eligible={deployment_eligible}")
