...
策略结构：
  ...
  + 类别同步确认
  + Relative Value 过滤
  + Movement Budget 运动空间相对成本过滤
  + Dynamic Exit 动态退出

重要说明：
    1. 当前脚本是规则策略，不是机器学习模型。
    2. 信号本身逐分钟更新。
    3. 参数和 ETF 适用范围默认每周滚动更新；也可以改为 daily/monthly。
...
默认输出：
    .\data_t0\backtest_combo_walk_forward_daily\

运行：
    python ".\backtest_combo_walk_forward_daily.py" `
      --panel-file ".\data_t0\processed\t0_intraday_bar_panel.parquet" `
      --out-dir ".\data_t0\backtest_combo_walk_forward_daily"

快速测试前 N 天：
    python ".\backtest_combo_walk_forward_daily.py" `
      --panel-file ".\data_t0\processed\t0_intraday_bar_panel.parquet" `
      --out-dir ".\data_t0\backtest_combo_walk_forward_debug" `
...
from t0_research.validation import clustered_mean_lower_bound
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", type=str, default="./data_t0/backtest_combo_walk_forward_daily")
    ...
    p.add_argument("--min-candidate-trades", type=int, default=30)
    p.add_argument("--min-candidate-days", type=int, default=20,
                   help="候选参数至少覆盖的历史交易日数，避免大量相关信号伪装成大样本。")
    p.add_argument("--min-etf-trades", type=int, default=10)
    p.add_argument("--min-etf-avg-net-bp", type=float, default=0.0)
    p.add_argument("--min-etf-profit-factor", type=float, default=1.0)
    p.add_argument("--fallback-all-if-no-etf-pass", action="store_true",
                   help="若历史窗口没有ETF通过筛选，则使用该candidate当天全部交易；默认不交易。")
    p.add_argument("--update-frequency", type=str, default="weekly", choices=["daily", "weekly", "monthly"],
    ...
    p.add_argument("--universe-min-history-days", type=int, default=20)
    p.add_argument("--universe-min-median-daily-amount", type=float, default=0.0,
                   help="动态标的池最低历史日成交额中位数，单位与分钟 amount 汇总后相同。")
...
def read_panel(path: Path) -> pd.DataFrame:
    ...
    # 兼容旧面板：这些列不应被简单补成全NaN，否则Noise和类别广度永远无信号。
    ...
    # 当前分钟累计 VWAP 只有收盘后才已知；下一根 bar 开盘时只能使用滞后一格的值。
    d["known_vwap"] = d.groupby(["ts_code", "trade_date"])["intraday_vwap"].shift(1)
    return d
...
def add_movement_budget(d: pd.DataFrame, p: ComboParams) -> pd.DataFrame:
    """Add a volatility/range budget; this is not a forecast of expected return."""
    x = d.copy()
    ...
    x["movement_budget_bp"] = np.maximum(x["or_range_bp"], x["noise_bp"]) + 0.5 * x["breakout_extra_bp"]
    x["movement_budget_ok"] = x["movement_budget_bp"] >= p.movement_cost_mult * p.assumed_roundtrip_cost_bp
    return x
...
def simulate_trade(day: pd.DataFrame, signal_pos: int, p: ComboParams) -> Dict:
    ...
    exit_result = simulate_long_exit(
        ...
        vwap_stop_band=p.vwap_stop_band,
        opening_range_stop_price=or_stop_price,
        stop_slippage_bp=p.stop_slippage_bp,
    )
    exit_pos = exit_result.bar_position
    ...
    return {
        ...
        "signal_movement_budget_bp": float(sig["movement_budget_bp"]) if np.isfinite(sig["movement_budget_bp"]) else np.nan,
    }
...
def apply_portfolio_constraints(trades: pd.DataFrame, p: ComboParams) -> pd.DataFrame:
    ...
    c = c.sort_values(
        ["entry_time", "signal_movement_budget_bp", "signal_rel_open_amount", "rv_rank_sort"],
...
def score_trades(trades: pd.DataFrame, cost_bp: float) -> Dict:
    if trades.empty:
        return {
            ...
            "trade_day_count": 0,
            "cluster_mean_net_bp": np.nan,
            "net_bp_cluster_se": np.nan,
            "net_bp_lcb_90": -np.inf,
            "score": -1e9,
        }
    ...
    n = int(len(trades))
    wr = float((net > 0).mean())
    cluster_mean_bp, lcb_90, n_days = clustered_mean_lower_bound(
        net * 10000.0,
        trades["trade_date"].astype(str),
    )
    daily_net_bp = pd.DataFrame({
        "trade_date": trades["trade_date"].astype(str).values,
        "net_bp": net.values * 10000.0,
    }).groupby("trade_date")["net_bp"].mean()
    cluster_se = float(daily_net_bp.std(ddof=1) / np.sqrt(n_days)) if n_days > 1 else np.nan
    # One-sided 90% lower confidence bound.  Parameter selection is deliberately
    # driven by uncertainty-adjusted net return, not by an unstable in-sample PF.
    lcb_90 = lcb_90 if np.isfinite(lcb_90) else -np.inf
    score = lcb_90
    ...
    return {
        ...
        "win_rate": wr,
        "profit_factor": pf,
        "trade_day_count": n_days,
        "cluster_mean_net_bp": cluster_mean_bp,
        "net_bp_cluster_se": cluster_se,
        "net_bp_lcb_90": lcb_90,
        "score": score,
    }
...
def select_candidate_and_etfs(
    ...
    score_df = score_df[
        (score_df["trade_count"] >= min_candidate_trades)
        & (score_df["trade_day_count"] >= min_candidate_days)
    ].copy()
    if score_df.empty:
        return {
            "selected": False,
...
def main() -> int:
    ...
    dynamic_universe = build_dynamic_universe(
        ...
        min_median_daily_amount=args.universe_min_median_daily_amount,
    )
    dynamic_universe.to_csv(out_dir / "wf_dynamic_universe.csv", index=False, encoding="utf-8-sig")
    panel = panel.merge(
        dynamic_universe[["ts_code", "trade_date", "universe_eligible"]],
        on=["ts_code", "trade_date"],
        how="left",
        validate="many_to_one",
    )
    panel = panel[panel["universe_eligible"].fillna(False)].copy()
    if panel.empty:
        raise RuntimeError("动态标的池为空，请检查历史长度或流动性阈值。")
