...
### 6.4 Walk-forward 样本外验证：禁用 ETF 二次筛选后的主线结果

项目进一步使用 daily walk-forward 方法进行滚动样本外验证。每个交易日只使用当前日前的历史窗口选择参数，然后在当天盘中交易。
...
早期版本曾加入“单 ETF 历史表现二次筛选”，即在选出 candidate 后，再根据过去窗口内单 ETF 的平均净收益和 profit factor 筛出当天可交易 ETF。后续拆解发现，该筛选层在样本外没有稳定预测力，反而削弱最终结果。因此，当前主线版本不再使用 ETF 历史收益二次筛选，仅保留 daily rolling candidate selection。
...
该结果说明：删除 ETF 二次筛选后，组合信号在 `commodity_focus` 池内恢复为弱正毛收益边际；但平均单笔毛收益只有约 0.84bp，一旦加入 1—2bp 单边成本，策略迅速转负。这表明当前信号仍不足以覆盖真实交易成本。
