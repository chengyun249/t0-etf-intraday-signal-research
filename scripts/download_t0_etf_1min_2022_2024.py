...
def write_selected_and_report(candidates: pd.DataFrame, val: pd.DataFrame, config_dir: Path, reports_dir: Path, args: argparse.Namespace) -> pd.DataFrame:
    ...
    x["has_enough_data"] = ((x["trading_days"] >= args.min_trading_days) & (x["rows"] > 0)).astype(int)

    if args.include_bond_money:
        x["selected_for_main"] = x["has_enough_data"]
    else:
        x["selected_for_main"] = (
            (x["has_enough_data"] == 1)
    ...
    lines.append(f"- selected_for_main: {len(selected)}")
    lines.append("")
    lines.append("## 3. 主策略池")
    ...
    lines.append("")
    lines.append("## 4. 策略建议")
    lines.append("本项目样本期固定为 2022-2024，其中 2022-2023 用于开发期，2024 用于测试期。债券/货币默认不进入主策略池，除非后续单独做低波动/低风险模块。")
